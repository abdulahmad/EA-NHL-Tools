# ANIM94SNES-To-BMP v0.1
Exports In-game sprites from NHL94 SNES ROM files to Photoshop RAW and Windows BMP format.
...
2. Go to the directory with the `animSnes94ToBmp.js` script

3. Run `npm i` to install node packages required for this script

4. Run `node animSnes94ToBmp <romfile>` or `node animSnes94ToBmp.js <romfile> Palettes\<Palette File>`, where the `<romfile>` is either the retail NHLPA93 v1.1 ROM or the retail NHL94 ROM for Sega Genesis.
